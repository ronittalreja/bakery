...
export function ExpensesTrackingPage({ onBack }: ExpensesTrackingPageProps) {
  ...
  const categories = [
    ...
    { value: "Return Charges", label: "Return Charges", color: "bg-red-200 text-red-900" },
    { value: "Packing Material", label: "Packing Material", color: "bg-orange-200 text-orange-900" },
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const expensesArray = Array.isArray(expensesData) ? expensesData : expensesData.expenses || expensesData.data || [];
        const mappedExpenses: Expense[] = expensesArray.map((row: any) => ({
          id: String(row.id),
        ...
        // Fetch credit notes for return charges
        ...
        // Fetch invoices for packing material
  ...
  // Calculate auto-generated expenses from credit notes and invoices
  useEffect(() => {
    const calculateAutoExpenses = async () => {
      console.log('Calculating auto expenses...');
      ...
      // Calculate return charges from credit notes (total loss value)
      const returnChargesTotal = creditNotes.reduce((sum, cn) => {
      ...
      console.log('Return charges total:', returnChargesTotal);

      if (returnChargesTotal > 0) {
        autoExpenses.push({
          id: `auto-return-charges-${selectedMonth}-${selectedYear}`,
          date: `${selectedYear}-${selectedMonth.toString().padStart(2, '0')}-01`,
          category: "Return Charges",
          description: `Total return charges from ${creditNotes.length} credit notes`,
          amount: returnChargesTotal,
          isAutoGenerated: true,
        });
        console.log('Added return charges expense');
      }

      // Calculate packing material expenses from invoice items
      let packingMaterialTotal = 0;
      let invoicesWithPackingMaterial = 0;
      ...
      for (const invoice of invoices) {
        try {
          ...
          if (response.ok) {
            ...
            // Filter items that are packing material
            const packingMaterialItems = items.filter((item: any) => {
              const category = item.category?.toLowerCase();
              return category === 'packing_material' || category === 'packaging' || category === 'packing';
            });
            
            if (packingMaterialItems.length > 0) {
              invoicesWithPackingMaterial++;
            }
            
            // Sum up their total prices
            const invoicePackingCost = packingMaterialItems.reduce((sum: number, item: any) => {
              return sum + (item.total_price || 0);
            }, 0);
            
            packingMaterialTotal += invoicePackingCost;
          }
        } catch (err) {
      ...
      console.log('Packing material total:', packingMaterialTotal);
      console.log('Invoices with packing material:', invoicesWithPackingMaterial);

      if (packingMaterialTotal > 0) {
        autoExpenses.push({
          id: `auto-packing-material-${selectedMonth}-${selectedYear}`,
          date: `${selectedYear}-${selectedMonth.toString().padStart(2, '0')}-01`,
          category: "Packing Material",
          description: `Total packing material costs from ${invoicesWithPackingMaterial} invoices`,
          amount: packingMaterialTotal,
          isAutoGenerated: true,
        });
        console.log('Added packing material expense');
      }

      console.log('Auto expenses calculated:', autoExpenses.length);

      // Merge auto-generated expenses with manual expenses
      // Remove existing auto-generated expenses for this month
      const manualExpenses = expenses.filter(e => !e.isAutoGenerated);
      const mergedExpenses = [...manualExpenses, ...autoExpenses];
      console.log('Total expenses after merge:', mergedExpenses.length);
      setExpenses(mergedExpenses);
    };

    calculateAutoExpenses();
  }, [creditNotes, invoices, selectedMonth, selectedYear, expenses]);
  ...
  const openEditDialog = (expense: Expense) => {
    ...
    if (expense.isAutoGenerated) {
      setError("Cannot edit auto-generated expenses");
      setTimeout(() => setError(""), 3000);
      return;
    }
  ...
  const handleDelete = async (expenseId: string) => {
    ...
    if (expense?.isAutoGenerated) {
      setError("Cannot delete auto-generated expenses");
      setTimeout(() => setError(""), 3000);
      return;
    }
                ...
                <Table>
                  ...
                  <TableBody>
                    {sortedExpenses.map((expense) => (
                      <TableRow key={expense.id} className={expense.isAutoGenerated ? "bg-blue-50/50" : ""}>
                        ...
                        <TableCell>
                          {expense.isAutoGenerated ? (
                            ...
                            <Badge className="bg-gray-100 text-gray-800 border-0" variant="secondary">
                              Manual
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEditDialog(expense)} disabled={isLoading || expense.isAutoGenerated}>
                              <Edit className="h-3 w-3" />
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => handleDelete(expense.id)} disabled={isLoading || expense.isAutoGenerated}>
