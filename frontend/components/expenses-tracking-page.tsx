...
export function ExpensesTrackingPage({ onBack }: ExpensesTrackingPageProps) {
  ...
  useEffect(() => {
    const calculateAutoExpenses = async () => {
      console.log('Calculating auto expenses using fast endpoints...');
      
      const autoExpenses: Expense[] = [];

      // Get total return charges from new backend endpoint
      try {
        ...
        const returnChargesResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/credit-notes/total-loss?month=${monthYear}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (returnChargesResponse.ok) {
          const data = await returnChargesResponse.json();
          if (data.success && data.totalLoss > 0) {
            autoExpenses.push({
              ...
              description: `Total return charges from ${data.creditNoteCount} credit notes`,
              amount: data.totalLoss,
              isAutoGenerated: true,
            });
            console.log('Added return charges expense:', data.totalLoss);
          }
        }
      } catch (err) {
        console.error('Error fetching total return charges:', err);
      }

      // Get total packing material costs from new backend endpoint
      try {
        const token = getAuthToken();
        if (!token) return;

        const packingMaterialResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/invoices/packing-material-total?month=${selectedMonth}&year=${selectedYear}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (packingMaterialResponse.ok) {
          const data = await packingMaterialResponse.json();
          if (data.success && data.totalPackingCost > 0) {
            autoExpenses.push({
              id: `auto-packing-material-${selectedMonth}-${selectedYear}`,
              date: `${selectedYear}-${selectedMonth.toString().padStart(2, '0')}-01`,
              category: "Packing Material",
              description: `Total packing material costs from ${data.invoiceCount} invoices`,
              amount: data.totalPackingCost,
              isAutoGenerated: true,
            });
            console.log('Added packing material expense:', data.totalPackingCost);
          }
        }
      } catch (err) {
        console.error('Error fetching total packing material costs:', err);
      }
